...
public class HomeworkController {
    ...
    public Result listHomeworks(@PathVariable(value = "pageNum", required = false) int pageNum,
        ...
        try {
            Page<Homework> page = homeworkService.selectPage(new Page<>(pageNum, 4),
                    new EntityWrapper<Homework>()
                            .eq("is_delete", 0)
     ...
     */
    @DeleteMapping("/{id}")
    public Result removeHomework(@PathVariable("id") Long id) {
        try {
            if (id != null && id != 0) {
    ...
    public Result removeHomeworkes(@RequestBody Map<String, Object> map) {
        ...
        List<Long> list = JSONObject.parseArray(map.get("ids").toString(), Long.class);
